# energoai_ssm_v7.py
"""
EnergoAI SSM v7.0 — Dual-Track SSM с адресацией памяти.
- Fast track: линейный scan с адаптивным забыванием
- Slow track: explicit memory slots с FIFO архивацией
- Intra-step attention: Fast читает Slow через learnable gate
- Hard State Constraint + Decoupled B + Forget Gate (наследие v6.5)
- Input gain: работает, масштабирует вход
- Per-loop decay: learnable, старт 0.5
"""
...
class SafeConfig:
    def __init__(self, raw_dict):
        ...
        self.num_prelude_layers = raw_dict.get("num_prelude_layers", 4)
        self.num_coda_layers = raw_dict.get("num_coda_layers", 4)
        self.num_loops = raw_dict.get("num_loops", 8)
...
# ==========================================
# 2. PRELUDE v7 (без изменений от v6.5)
# ==========================================
...
class EnergoAIDecoderLayerV7(nn.Module):
    def __init__(self, config, intermediate_multiplier=6):
        super().__init__()
...
class SlowTrackMemory(nn.Module):
    def __init__(self, state_dim, hidden_dim, num_slots=32, surprise_threshold=0.1):
        super().__init__()
        self.N = state_dim      # 128
        self.D = hidden_dim     # 256
        self.K = num_slots
    ...
    def forward(self, h_f, slots, h_prev=None):
        ...
        if h_prev is None:
            return r_t, g_t, slots, h_f.detach()

        surprise = torch.norm(h_f - h_prev, dim=-1)
        prev_norm = h_prev.norm(dim=-1) + 1e-6
        relative_change = surprise / prev_norm

        mask = (relative_change > self.surprise_threshold).float().view(-1, 1, 1)

        compressed = self.compress(h_f)
        ...
        new_h_prev = h_f.detach()
        return r_t, g_t, updated_slots, new_h_prev


# ==========================================
# 4. DUAL-TRACK SSM CORE v7
# ==========================================
class DualTrackSSMCore(nn.Module):
    def __init__(self, hidden_dim, state_dim=128, num_slots=32):
        super().__init__()
        self.D = hidden_dim
        self.N = state_dim
        self.K = num_slots
        ...
        self.b_Δ = nn.Parameter(torch.logspace(-4, -1, self.N))
        ...
        self.slow_track = SlowTrackMemory(
            state_dim=self.N,  # 128
            hidden_dim=self.D,  # 256
            num_slots=self.K  # 32
        )
        self.W_r = nn.Linear(self.D, self.N, bias=False)
    ...
    def _init_weights(self):
        with torch.no_grad():
            for lin in [self.proj_B, self.proj_C, self.W_out, self.W_gate, self.W_r]:
    ...
    def forward(self, x, h_prev, slots=None, h_prev_slow=None,
                depth_emb_t=None, mask=None, return_delta=False):
        B, L, _ = x.shape
        ...
        input_term = self.input_gain * x_B_raw
        ...
        Δ = torch.exp(dt_logits)

        A = -torch.exp(self.A_log)

        exponent = torch.clamp(Δ * A.view(1, 1, -1), min=-10.0, max=0.0)
        A_bar = torch.exp(exponent)

        G = torch.sigmoid(self.proj_G(x_norm))

        # Slow track read (один раз на входе, h_prev как query)
        r_t = None
        g_t = None
        slots_new = slots
        h_slow_new = h_prev_slow

        if slots is not None and h_prev_slow is not None:
            r_t, g_t, slots_new, h_slow_new = self.slow_track(h_prev, slots, h_prev_slow)
            r_projected = self.W_r(r_t).unsqueeze(1).expand(-1, L, -1)
            input_term = input_term + g_t.unsqueeze(1) * r_projected

        h = h_prev
        h_list = []
        tau = F.softplus(self.state_tau) + 5.0
        ...
        # Slow track update (финальное h как новый архив)
        if slots is not None:
            h_depth_out = h_seq[:, -1, :] if mask is None else h_seq[torch.arange(B, device=device), (mask.sum(dim=1) - 1).clamp(min=0, max=L - 1), :]
            _, _, slots_final, h_slow_final = self.slow_track(h_depth_out, slots_new, h_slow_new)
        else:
            slots_final = None
            h_slow_final = None
            h_depth_out = h_seq[:, -1, :]
        ...
        if return_delta:
            ...
            with torch.no_grad():
                self.diagnostics = {
                    ...
                    "slots_used": (slots_final.norm(dim=-1) > 0.01).sum().item() if slots_final is not None else 0,
                    "g_t_mean": g_t.mean().item() if g_t is not None else 0,
                    "r_t_norm": r_t.norm().item() if r_t is not None else 0,
                }
            return delta, h_depth_out, slots_final, h_slow_final
        else:
            x_out = x + delta
            if mask is not None:
                x_out = torch.where(mask.unsqueeze(-1), x_out, x)

            return x_out, h_depth_out, slots_final, h_slow_final


# ==========================================
# 5. CHECKPOINT SSM v7 (с slow track)
# ==========================================
class CheckpointSSM(nn.Module):
    ...
    def forward(self, x, h_depth_in, slots=None, h_prev_slow=None, **kwargs):
        B, L, _ = x.shape
        self.checkpoints = []
        ...
        for start in range(0, L, self.checkpoint_every):
            ...
            out_chunk, h_prev, slots_current, h_slow_current = self.ssm(
                x_chunk, h_prev,
                slots=slots_current,
                h_prev_slow=h_slow_current,
                **kwargs_chunk
            )
        ...
        return x_out, h_prev, slots_current, h_slow_current
...
class CodaBlockWithCheckpoints(CodaBlock):
    ...
    def forward(self, hidden_states, state_bias, checkpoints=None):
        ...
        if checkpoints is not None and checkpoints.shape[0] > 0:
            ...
            Q = self.checkpoint_q(hidden_states)
            ckpt_perm = checkpoints.permute(1, 0, 2)
            K = self.checkpoint_k(ckpt_perm)
            V = self.checkpoint_v(ckpt_perm)

            scores = torch.einsum('bld,bnd->bln', Q, K) / 8.0
            attn_weights = F.softmax(scores, dim=-1)
            checkpoint_context = torch.einsum('bln,bnd->bld', attn_weights, V)

            gate = torch.sigmoid(self.checkpoint_gate(hidden_states))
...
# 8. ENERGOAI MODEL v7
# ==========================================
class EnergoAIModelV7(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        self.prelude = nn.ModuleList([
            EnergoAIDecoderLayerV7(config) for _ in range(int(config.num_prelude_layers))
        ])
        ...
        self.core_block = CheckpointSSM(
            DualTrackSSMCore(
                ...
                num_slots=int(getattr(config, 'num_slots', 32))
            ),
            checkpoint_every=int(getattr(config, 'checkpoint_every', 1024))
        )
        ...
        # Slow track init
        self.slots_init = nn.Parameter(torch.zeros(1, 32, int(config.hidden_size)))  # [1, 32, 256]
        self.h_slow_init = nn.Parameter(torch.zeros(1, int(config.state_dim)))

        # Per-loop decay: learnable, старт 0.5
        self.depth_decay_logits = nn.ParameterList([
            nn.Parameter(torch.zeros(int(config.state_dim)))
    ...
    def forward_with_diagnostics(self, input_ids, attention_mask=None):
        ...
        slots = self.slots_init.expand(bsz, -1, -1).clone()
        h_slow = self.h_slow_init.expand(bsz, -1).clone()

        for t in range(self.num_loops):
            depth_emb_t = self.depth_emb[t]

            delta, h_depth, slots, h_slow = self.core_block(
            ...
            decay = torch.sigmoid(self.depth_decay_logits[t])
        ...
        memory_signal = self.memory_skip(h_depth_norm).unsqueeze(1)
        ...
        diagnostics = {
            ...
            "slots_final_norm": round(slots.norm(dim=-1).mean().item(), 3),
            "h_slow_norm": round(h_slow.norm(dim=-1).mean().item(), 3),
    ...
    def from_pretrained(cls, model_path):
        ...
        print("✅ Веса загружены. Tied weights восстановлены. v7.0 готова.")
        return model