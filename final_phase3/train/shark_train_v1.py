"""
Быстрое обучение EnergoAI v3 на Tiny Shakespeare (~150 шагов)
Запуск: python train_v3_quick.py
"""
import torch
    ...
    for epoch in range(3):
        for batch_idx, (input_ids, targets) in enumerate(dataloader):
            ...
            if loop_stats:
                ...
                avg_h_norm_fixed = sum(s.get('h_depth_norm_after_fix', 0) for s in loop_stats) / len(loop_stats)
                ...
                metrics['h_depth_norm_fixed'].append(avg_h_norm_fixed)   # ← новая метрика
            ...
            if step % 10 == 0 or step == total_steps - 1:
                print(f"Step {step:3d} | Loss: {loss.item():.4f} | "
                      ...
                      f"Δ_norm: {metrics['delta_norm'][-1]:.4f} | "
...
def plot_training_metrics(metrics):
    fig, axes = plt.subplots(2, 4, figsize=(18, 8))   # 2x4
    ...
    # 5. H Depth Norm (сырая)
    axes[1, 0].plot(metrics['step'], metrics['h_depth_norm'], 'purple', linewidth=2)
    axes[1, 0].set_title('H Depth Norm (raw)')
    axes[1, 0].grid(True)

    # 6. H Depth Norm (после фиксации)
    axes[1, 1].plot(metrics['step'], metrics['h_depth_norm_fixed'], 'cyan', linewidth=2)
    axes[1, 1].set_title('H Depth Norm (fixed)')
    axes[1, 1].set_ylim(0, 20)   # ожидаем 15
    axes[1, 1].grid(True)

    # 7. Loss (smoothed)
    if len(metrics['loss']) > 10:
        smoothed = [sum(metrics['loss'][max(0, i - 5):i + 1]) / min(i + 1, 6) for i in range(len(metrics['loss']))]
        axes[1, 2].plot(metrics['step'], smoothed, 'b-', linewidth=2, label='Smoothed')
        axes[1, 2].plot(metrics['step'], metrics['loss'], 'b-', alpha=0.3, label='Raw')
        axes[1, 2].legend()
    axes[1, 2].set_title('Loss (Smoothed)')
    axes[1, 2].grid(True)

    # 8. Пустой (или можно убрать)
    axes[1, 3].axis('off')

    plt.suptitle("EnergoAI v3 Training Metrics (150 steps)", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig("training_metrics_v3.png", dpi=150)
    print("📈 Графики сохранены в training_metrics_v3.png")
    plt.show()
