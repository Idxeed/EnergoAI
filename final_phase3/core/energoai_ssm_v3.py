# energoai_ssm_v5.py
"""
EnergoAI SSM Model v3
- Устойчивые петли глубины с остаточной связью
- Обучаемый decay состояния и нормализация h_depth
- SSM в режиме delta (только добавка)
- Без detach от Prelude
- Подготовка к динамической глубине (3-12 циклов)
"""
...
import os
import json
from final_phase3.layers.coda_layer import StateConditionedMLP  # оставляем импорт из внешнего файла
        ...
        if return_delta:
            ...
            with torch.no_grad():
                self.diagnostics = {
                    ...
                    "gate_norm": gate.norm(dim=-1).mean().item(),  # ← добавили
                    "y_norm": y.norm(dim=-1).mean().item(),  # ← добавили
...
class EnergoAIModel(nn.Module):
    def __init__(self, config):
        ...
        self.prelude = nn.ModuleList([EnergoAIDecoderLayer(config) for _ in range(2)])

        # SSM ядро с чекпойнтами
        self.core_block = CheckpointSSM(
            SSMLayer(int(config.hidden_size), state_dim=128),
        ...
        self.h_depth_init = nn.Parameter(torch.zeros(1, 128))
        with torch.no_grad(): self.h_depth_init.normal_(0, 0.01)

        # Обучаемый decay состояния
        self.depth_decay_logit = nn.Parameter(torch.tensor(3.0))  # sigmoid ≈ 0.95

        # Нормализация h_depth после каждого цикла
        self.h_depth_norms = nn.ModuleList([
            RMSNorm(128, eps=float(config.rms_norm_eps)) for _ in range(self.num_loops)
        ])
        ...
        nn.init.xavier_uniform_(self.bridge[1].weight, gain=0.1)
        ...
        self.output_head = FlexibleOutputHead(config, memory_dim=128)
    ...
    def forward_with_diagnostics(self, input_ids, attention_mask=None):
        ...
        for t in range(self.num_loops):
            ...
            h_depth = F.normalize(h_depth, dim=-1) * 15.0  # жёстко 15

            loop_diag = dict(self.core_block.ssm.diagnostics)
            loop_diag["loop"] = t
            loop_diag["decay"] = decay.item()
        ...
        hidden_states = self.coda(hidden_states, h_depth, checkpoints)
        ...
        diagnostics = {
            ...
            "depth_decay": torch.sigmoid(self.depth_decay_logit).item()
        }
